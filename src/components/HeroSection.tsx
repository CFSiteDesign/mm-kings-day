...
import cupidSvg from "@/assets/cupid.png";
  ...
  return (
    <section className="relative z-10 w-full min-h-[100svh] md:min-h-[85vh] flex flex-col md:flex-row border-b-4 border-divider overflow-visible">
      ...
      <div className="flex-1 bg-background px-5 pt-2 pb-6 md:p-12 flex flex-col justify-center items-start border-b-4 md:border-b-0 md:border-r-4 border-divider relative">
        ...
        <div className="mb-4 md:mb-6 inline-flex items-center gap-2 bg-primary text-primary-foreground font-black uppercase px-4 py-2 text-xs md:text-lg border-2 border-primary -rotate-2 z-10 rounded-sm" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.5)' }}>
          March 17th 2026
        </div>
        <h1 className="text-5xl md:text-8xl lg:text-9xl font-black uppercase leading-[0.85] mb-5 md:mb-8 z-10">
          GET LUCKY.<br />
          <span className="text-primary">DRINK GREEN.</span>
        </h1>
        <p className="text-[13px] md:text-2xl font-bold mb-6 md:mb-10 max-w-xl leading-relaxed z-10">
          Roses are red, the beer's dyed green,<br />
          The shamrocks are out and we're living the dream.<br />
          The craic is mighty, we're going full send,<br />
          No saints, just sessions, DJs, and pints without&nbsp;end.
        </p>
        <a
          href="#selector"
      ...
      <div className="flex-1 bg-background relative flex items-center justify-center min-h-[300px] md:min-h-[400px]">
        ...
        {/* Shamrock - inside hero on mobile, floating out bottom on desktop */}
        <img
          src={cupidSvg}
          alt="Shamrock"
